...
enum e_content // перечесление целочисленные в либу
{
	SA,
	SB,
	SS,
...
void	check_top_value(t_list **a, t_list **b, t_check *info, t_list **comms);
 // проверка топ стека со следующим значением (оптимизация основного кода)
 int			check_values(t_list *s, int value);//проверка значения,???
 //void		create_array(int (**arr)(t_list **, t_list **)); //созндание массива команд для чекера
#endif